          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (!make_spte_executable (file, file_page, (void *) mem_page,
...
/* Checks whether PHDR describes a valid, loadable segment in
   EXECUTABLE and returns true if so, false otherwise. */
static bool
validate_segment (const struct Elf32_Phdr *phdr, struct file *file) 
{
  ...
  /* p_offset must point within EXECUTABLE. */
...
/* Loads a segment starting at offset OFS in EXECUTABLE at address
   UPAGE.  In total, READ_BYTES + ZERO_BYTES bytes of virtual
   memory are initialized, as follows:

        - READ_BYTES bytes at UPAGE must be read from EXECUTABLE
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      /* Calculate how to fill this page.
         We will read PAGE_READ_BYTES bytes from EXECUTABLE
         and zero the final PAGE_ZERO_BYTES bytes. */
      size_t page_read_bytes = read_bytes < PGSIZE ? read_bytes : PGSIZE;
      size_t page_zero_bytes = PGSIZE - page_read_bytes;

      /* Get a page of memory. */
      uint8_t *kpage = allot_frame (PAL_USER);
      if (kpage == NULL)
        return false;
...
setup_stack (void **esp, char *file_name, char *args) 
{
  ...
  bool success = false;
  /*
  kpage = allot_frame (PAL_USER | PAL_ZERO);
  if (kpage != NULL) 
    {
